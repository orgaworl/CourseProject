#pragma once
#include<ctime>
#include<iostream>
//using namespace std;

typedef unsigned char uint8;
typedef unsigned short int uint16;
typedef unsigned int uint32;

void test_SM4();
void benchmark_SM4(int tstLen = 16);

int SM4KeyGen8(uint8 key[16], uint32 RoundKeySet[36]);
int SM4KeyGen32(uint32 key[4], uint32 RoundKeySet[36]);
void SM4Encrypt8ECB(uint8* plain, int length, uint8 Key[16], uint8* cipher);
void SM4Encrypt8(uint8 plain8[16], uint8 key8[16], uint8 cipher8[16]);
void SM4Encrypt32(uint32 plain32[4], uint32 key32[4], uint32 cipher32[4]);


const uint32 FK[4] = {
    0xA3B1BAC6 ,
    0x56AA3350 ,
    0x677D9197 ,
    0xB27022DC
};

const uint8  SM4SBox[256] = {
        
};

//Key 50 48 50 49 48 48 52 54 48 49 49 54



